...
"Microsoft Build Engine" はアプリケーションを作成するのに必要なコンパイラとかが入った「プラットフォーム」です。一般に"MSBuild"として知られています。
通常アプリケーションを作成する場合はVisualStudioにおいてプログラムをコンパイルします。その際に実はMSBuildEngineが使われているわけです。（MSBuild（=msbuild.exe）は単体でも動きます。）これはつまり、VSisualStudioがなくてもMSBuildさえ動けばアプリケーションが作成できるということです。そのMSBuildが今回オープンソースになりました！
コンパイラではなく「プラットフォーム」といいましたが、ほかに何が入っているかというと、たとえばVisualStudioはアプリケーション作成プロジェクトの情報をXMLでプロジェクトファイルとして使用しますが、そのためのXMLスキーマ（XML記述のルール、構造みたいなのの定義をするもの）もMSBuildに入ってます。

詳しくはこちら（aboutMSBuild）＞＞＞[MSDN documentation](https://msdn.microsoft.com/en-us/library/dd393574(v=vs.120).aspx).
